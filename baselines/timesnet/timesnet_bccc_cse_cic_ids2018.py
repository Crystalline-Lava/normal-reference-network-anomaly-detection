...
"""TimesNet baseline for the BCCC-CSE-CIC-IDS2018 entity-window task.
...
are disabled. The model uses the official PSM-style TimesNet configuration and
trains for six epochs, while retaining the official three-epoch budget result.
...
# TimesNet-specific configuration

from dataclasses import dataclass  # noqa: E402  (TimesNetConfig)
import torch.fft  # noqa: E402  (fft_for_period)
...
EPOCHS = 6
OFFICIAL_BUDGET_EPOCHS = 3       # official TimesNet AD budget row (@off3ep)
BATCH_SIZE = 128
LR = 1e-4                        # Adam, halved every epoch (lradj type1)
PATIENCE = 3
MIN_SELECT_EPOCH = 1
READOUTS = ["raw", "std", "maha"]  # raw = E.mean(1) IS the native TSLib score

# Official PSM-style TimesNet configuration
D_MODEL = 64
D_FF = 64
E_LAYERS = 2
TOP_K = 3
...
EXPERIMENT_GROUP = "timesnet_bccc_cse_cic_ids2018"
...
# ═══════════════════════════════════════════════════════════════════════════════
#  TimesNet model stack + readout helpers — based on the official implementation
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
#  TimesNet baseline glue — output roots, streaming scorers, selection loop
# ═══════════════════════════════════════════════════════════════════════════════

# Flat output naming inside the baseline group (the transformer arch tag from
# the proposed pipeline is not used for TimesNet):
# .../timesnet_bccc_cse_cic_ids2018/seed_<s>/
def architecture_tag() -> str:  # override
    return (f"tn_dm{D_MODEL}_ff{D_FF}_el{E_LAYERS}_k{TOP_K}"
...
class TupleWindowDataset(ArrayWindowDataset):
    """ArrayWindowDataset wrapped to yield 1-tuples so the TimesNet baseline's
    train_one_epoch/validate loops (`for (batch_x,) in loader`) work on the
...
def _score_split_stream(model: nn.Module, split_ctx: Dict[str, Any],
    ...
    """Stream a split through TimesNet; return (E, y_attack, y_family) with E
    ...
    E.mean(1) is the native TSLib score."""
    model.eval()
...
def run_experiment(run_seed):
    ...
    with open(os.path.join(OUTPUT_DIR, "run_config.json"), "w") as f:
        json.dump({
            ...
            "top_k": TOP_K, "num_kernels": NUM_KERNELS, "dropout": DROPOUT_TN,
            "window_size": WINDOW_SIZE,
            "train_stride": TRAIN_STRIDE,
            "test_stride": TEST_STRIDE,
    ...
    # TimesNet construction (official PSM recipe)
    print("\n" + "=" * 80)
    print("  TimesNet Baseline Construction (official PSM config)")
    print("=" * 80)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if loss_improved:
            ...
            patience_ctr = 0
            captures["nat"] = _capture()      # NATIVE: TSLib's own rule
...
def main():
    ...
    all_results = []
    for s in RUN_SEEDS:
        print(f"\n\n{'=' * 80}\n  STARTING TIMESNET BCCC-CSE-CIC-IDS2018 RUN WITH SEED {s}"
              f"\n{'=' * 80}")
        all_results.append(run_experiment(s))

    print(f"\n\n{'=' * 80}\n  AGGREGATED TIMESNET RESULTS OVER "
