...
"""USAD baseline for the BCCC-CSE-CIC-IDS2018 entity-window task.
...
are disabled. USAD uses MinMax scaling because its sigmoid decoders require
targets in [0, 1]. Windows are flattened per batch and trained for the fixed
12-epoch USAD budget with two Adam optimizers.

To keep the model tractable at this dataset's input width, hidden layers use
the absolute widths from the CIC-UNSW-NB15 USAD configuration (1675 and 837);
only the input projection changes with feature count. Results report the final
epoch selected by the native USAD rule and a validation-AP-selected comparison.
The split fingerprint is checked on the first run to prevent data drift.
"""
...
# USAD-specific configuration

from sklearn.preprocessing import MinMaxScaler  # noqa: E402  (USAD native scaling)
...
EPOCHS = 12
# No separate official-budget row: USAD trains for a fixed number of epochs
# without early stopping. The final epoch is the native checkpoint (``nat`` is
# overwritten unconditionally every epoch). There is no smaller official
# budget to restrict to, hence None (serialized into run_config.json).
OFFICIAL_BUDGET_EPOCHS = None
BATCH_SIZE = 256
# Adam uses its framework-default learning rate; LR is documentation only.
LR = 1e-3
MIN_SELECT_EPOCH = 1
READOUTS = ["raw", "std", "maha"]  # raw = E.mean(1) at alpha = beta = 0.5
USAD_ALPHA = 0.5
USAD_BETA = 0.5

# Dimensional adaptation: reuse the absolute hidden and latent widths from the
# CIC-UNSW-NB15 configuration; only the input projection follows feature count.
CIC_UNSW_NB15_W_SIZE = 3350      # 50 flows × 67 CIC-UNSW-NB15 features
USAD_HIDDEN_WIDTHS = [1675, 837]
USAD_Z_SIZE = 837

EXPERIMENT_GROUP = "usad_bccc_cse_cic_ids2018"
...
def load_and_preprocess():
    ...
    cache_key = stable_cache_key(
        "preproc",
        {
            ...
            # USAD keeps its NATIVE MinMax [0,1] scaling (sigmoid output
            # decoders) — different preprocessed values than the
            # StandardScaler baselines; force a disjoint cache.
            "usad_native_scaling": True,
        },
    )
    ...
    if stage_cache_complete(cached_scaler):
        ...
        if cached_scaler is not None:
            ...
            # USAD native scaling (disclosed asymmetry): MinMax to [0,1]
            # as in the CIC-UNSW-NB15 USAD configuration — fit on train-benign
            # flows, clip=True reproduces the native
            # np.clip(X_scaled, 0.0, 1.0) on transform. Sigmoid output
            # decoders match the [0,1] range.
            scaler = MinMaxScaler(clip=True)
            processed_scaler_paths = set()
        print("\nStreaming train-benign traffic to fit MinMaxScaler (USAD native scaling) ...")
        for file_spec in split_plan["train_benign"]:
            path_key = str(file_spec["path"])
...
# ═══════════════════════════════════════════════════════════════════════════════
#  USAD model stack and readout helpers
# ═══════════════════════════════════════════════════════════════════════════════
...
class EncoderPinned(nn.Module):
    """USAD encoder with explicit hidden widths instead of input-width halving."""

    def __init__(self, in_size, hidden_widths, latent_size):
        super().__init__()
        widths = [in_size] + list(hidden_widths) + [latent_size]
...
class DecoderPinned(nn.Module):
    """USAD decoder with explicit widths and a sigmoid output for [0, 1] inputs."""

    def __init__(self, latent_size, hidden_widths, out_size):
        super().__init__()
        widths = [latent_size] + list(hidden_widths) + [out_size]
        self.layers = nn.ModuleList(
            nn.Linear(a, b) for a, b in zip(widths[:-1], widths[1:]))
        self.relu = nn.ReLU(True)
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, z):
        ...
        for layer in self.layers[:-1]:
            out = self.relu(layer(out))
        return self.sigmoid(self.layers[-1](out))


class UsadModelPinned(UsadModel):
    """USAD with fixed hidden widths from the CIC-UNSW-NB15 configuration.

    Only the input projection changes with feature count. Training and
    validation retain the standard adversarial USAD losses from ``UsadModel``.
    """

    def __init__(self, w_size, z_size, hidden_widths):
        nn.Module.__init__(self)
        self.encoder = EncoderPinned(w_size, hidden_widths, z_size)
        self.decoder1 = DecoderPinned(z_size, list(reversed(hidden_widths)),
                                      w_size)
        self.decoder2 = DecoderPinned(z_size, list(reversed(hidden_widths)),
                                      w_size)


@torch.no_grad()
def usad_validate(model: nn.Module, loader: DataLoader, n: int,
                  device: torch.device) -> Tuple[float, float]:
    """Return mean benign-validation USAD losses for training diagnostics."""
    model.eval()
    outputs = []
    for (batch_x,) in loader:
        batch = batch_x.float().to(device).view(batch_x.size(0), -1)
        outputs.append(model.validation_step(batch, n))
    return (
        float(torch.stack([o["val_loss1"] for o in outputs]).mean().item()),
        float(torch.stack([o["val_loss2"] for o in outputs]).mean().item()),
    )
...
def feature_residuals(model: nn.Module, x: np.ndarray, device: torch.device,
    ...
    """Compute the per-window, per-feature USAD residual matrix.

    The two decoder residuals are averaged with equal weights, reshaped from
    flattened windows to ``(window, flow, feature)``, and averaged over flows.
    The raw score is the feature mean; calibrated readouts use the same matrix.
    """
    model.eval()
    _, W, Fdim = x.shape
    ...
    out: List[np.ndarray] = []
    for (batch_x,) in loader:
        batch = batch_x.to(device).view(batch_x.size(0), -1)
        w1 = model.decoder1(model.encoder(batch))
        w2 = model.decoder2(model.encoder(w1))
        r = 0.5 * (batch - w1) ** 2 + 0.5 * (batch - w2) ** 2
        out.append(r.view(-1, W, Fdim).mean(1).cpu().numpy())
...
# ═══════════════════════════════════════════════════════════════════════════════
#  USAD baseline glue — output roots, streaming scorers, selection loop
# ═══════════════════════════════════════════════════════════════════════════════

# Flat output naming inside the baseline group: .../usad_bccc_cse_cic_ids2018/seed_<s>/
def architecture_tag() -> str:  # override
    return (f"usad_h{'x'.join(str(h) for h in USAD_HIDDEN_WIDTHS)}"
            f"_z{USAD_Z_SIZE}_w{WINDOW_SIZE}_d{int(USE_DELTA_FEATURES)}")
...
class TupleWindowDataset(ArrayWindowDataset):
    """ArrayWindowDataset wrapped to yield 1-tuples so the USAD baseline's
    training/validation loops (`for (batch_x,) in loader`) work on the
...
def _score_split_stream(model: nn.Module, split_ctx: Dict[str, Any],
    ...
    """Stream a split through USAD; return (E, y_attack, y_family) with E
    the per-window per-feature residual (N, F) = mean over W of
    0.5·(x−AE1(x))² + 0.5·(x−AE2(AE1(x)))² on the FLATTENED window — exactly
    the tensor feature_residuals() produces for in-memory arrays, so E.mean(1)
    is the paper score at alpha=beta=0.5."""
    model.eval()
    ...
    for X_w, y_attack, y_family in _iter_split_window_batches(split_ctx, preproc_ctx):
        ...
        for i in range(0, len(X_w), EVAL_MODEL_BATCH):
            ...
            b = bx.view(bx.size(0), -1)  # flatten at batch time only
            w1 = model.decoder1(model.encoder(b))
            w2 = model.decoder2(model.encoder(w1))
            r = 0.5 * (b - w1) ** 2 + 0.5 * (b - w2) ** 2
            E_parts.append(r.view(b.size(0), WINDOW_SIZE, -1).mean(1)
...
def run_experiment(run_seed):
    ...
    with open(os.path.join(OUTPUT_DIR, "run_config.json"), "w") as f:
        json.dump({
            ...
            "usad_hidden_widths": USAD_HIDDEN_WIDTHS,
            "usad_z_size": USAD_Z_SIZE,
            "cic_unsw_nb15_w_size_pinned": CIC_UNSW_NB15_W_SIZE,
            "usad_alpha": USAD_ALPHA, "usad_beta": USAD_BETA,
            "window_size": WINDOW_SIZE,
            "train_stride": TRAIN_STRIDE,
            "test_stride": TEST_STRIDE,
    ...
    # USAD construction (official recipe; B-variant pinned widths)
    print("\n" + "=" * 80)
    print("  USAD baseline construction (fixed reference widths)")
    print("=" * 80)
    w_size = WINDOW_SIZE * n_features
    model = UsadModelPinned(w_size, USAD_Z_SIZE, USAD_HIDDEN_WIDTHS).to(device)
    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"  Parameters: {n_params:,} (hidden {USAD_HIDDEN_WIDTHS}, "
          f"z {USAD_Z_SIZE} — CIC-UNSW-NB15 reference widths)")
    print(f"  Flattened window size: {w_size} "
          f"({WINDOW_SIZE} flows x {n_features} features, delta OFF; "
          f"flattened per batch only)")
    print(f"  Score: alpha={USAD_ALPHA}, beta={USAD_BETA}")
    ...
    val_loader = DataLoader(TupleWindowDataset(X_val_w),
                            batch_size=BATCH_SIZE, shuffle=False)
    # Two optimizers use torch.optim.Adam's framework-default learning rate.
    ...
    # Training and checkpoint selection (final epoch or validation AP per readout)
    print("\n" + "=" * 80)
    print("  Training (USAD adversarial two-phase) + per-epoch readout tracking")
    print(f"  Selection: native = FINAL epoch (official USAD: fixed "
          f"{EPOCHS}-epoch budget, no early stop); "
    ...
    for epoch in range(1, EPOCHS + 1):
        t0 = time.time()
        # USAD adversarial two-phase step (two forward passes per
        # batch, one per optimizer; 1/n epoch weighting inside
        # training_step). Windows flatten at batch time only.
        model.train()
        ...
        val_loss1, val_loss2 = usad_validate(model, val_loader, epoch, device)
        ...
        # NATIVE rule (USAD): official USAD trains a fixed N-epoch budget
        # with no early stop and no loss-based checkpointing — the FINAL
        # epoch IS the native model. Overwrite unconditionally every epoch
        # so captures["nat"] always holds the last completed epoch. No
        # @off row (no separate official budget beyond EPOCHS itself).
        captures["nat"] = _capture()
        for r in READOUTS:
        ...
        history.append({"epoch": epoch, "val_loss1": val_loss1,
                        "val_loss2": val_loss2, "time_s": elapsed,
                        **{f"val_ap_{r}": vaps[r] for r in READOUTS}})
        print(f"  Epoch {epoch:3d}/{EPOCHS}  loss1={val_loss1:.6f}  "
              f"loss2={val_loss2:.6f}  val_AP raw {vaps['raw']:.4f} "
              f"std {vaps['std']:.4f} maha {vaps['maha']:.4f}  "
              f"({elapsed:.1f}s)")

        # No early stop: official USAD always trains the full fixed budget.
    ...
    print("  (raw = E.mean(1), the reported score at alpha=beta=0.5;")
    print("  native selection uses the final fixed-budget epoch, while")
    print("  valAP-sel uses validation AP.)")
    print("=" * 80)
    print(f"  {'readout':<18}{'sel_ep':>7}{'val_AP':>9}{'test_AP':>9}"
          f"{'AUROC':>8}{'ho_AP':>8}{'TPR@1%':>8}{'FPR_act':>9}{'hoTPR':>7}")
    print("  " + "-" * 84)
    rows = {}
    rules = [("natsel", "nat", "nat-final"),
...
def main():
    ...
    all_results = []
    for s in RUN_SEEDS:
        print(f"\n\n{'=' * 80}\n  STARTING USAD BCCC-CSE-CIC-IDS2018 RUN WITH SEED {s}"
              f"\n{'=' * 80}")
        all_results.append(run_experiment(s))

    print(f"\n\n{'=' * 80}\n  AGGREGATED USAD RESULTS OVER "
          f"{len(RUN_SEEDS)} SEEDS (reported protocol)\n{'=' * 80}")
    agg_keys = ([f"{r}_natsel" for r in READOUTS]      # PRIMARY (final epoch)
